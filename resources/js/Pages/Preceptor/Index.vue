...
const metricasCards = computed(() => [
    ...
    {
        titulo: 'Asistencias',
        valor: props.metricas?.asistencias_hoy || 0,
        subtitulo: 'Cargadas hoy',
...
const accesosRapidos = computed(() => {
    const accesos = [
        {
            titulo: 'Expediente',
            descripcion: 'Gestionar legajos academicos',
...
<template>
    <Head title="Panel de Preceptor" />

    <SidebarLayout :user="$page.props.auth.user">
        <template #header>
            <div>
                <h1 class="text-xl font-semibold text-white">Panel de Preceptor</h1>
        ...
        <div class="p-4 md:p-8 max-w-7xl mx-auto">
            <!-- Banner -->
            <div class="bg-gradient-to-r from-amber-600 via-orange-600 to-red-600 rounded-lg md:rounded-xl p-4 md:p-8 mb-4 md:mb-8 text-white relative overflow-hidden">
                <div class="relative z-10 flex items-center justify-between">
                    <div class="flex-1 min-w-0">
                        <h2 class="text-xl md:text-3xl font-bold mb-1 md:mb-2 truncate">{{ obtenerSaludo() }}, {{ user.nombre }}</h2>
                        <p class="text-amber-100 text-sm md:text-lg">Panel de Preceptor</p>
                        <div v-if="metricas.periodo_activo" class="mt-2 text-xs md:text-sm text-amber-100">
                    ...
                    <div class="hidden md:block flex-shrink-0 ml-4">
                        <div class="w-20 h-20 md:w-32 md:h-32 bg-white/10 rounded-full flex items-center justify-center backdrop-blur-sm">
                            <i class="bx bx-calendar-check text-4xl md:text-6xl"></i>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Metricas -->
            <div class="grid grid-cols-2 gap-2 md:gap-4 mb-6 md:mb-8">
                <div
            ...
            <div class="mb-6">
                <h3 class="text-lg md:text-xl font-semibold text-gray-900 mb-3 md:mb-4 flex items-center">
                    <i class="bx bx-grid-alt text-xl md:text-2xl mr-2 text-orange-600"></i>
                    Accesos Rapidos
                </h3>
                        ...
                        >
                            <div :class="['bg-white border-2 border-gray-200 rounded-lg md:rounded-xl p-4 md:p-6 transition-all duration-200 hover:shadow-xl', acceso.hoverBorder]">
                                <div class="flex items-center mb-2 md:mb-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="text-base md:text-lg font-semibold text-gray-900 group-hover:text-orange-600 transition-colors truncate">{{ acceso.titulo }}</h3>
                                    </div>
                                    <i class="bx bx-chevron-right text-xl md:text-2xl text-gray-400 group-hover:text-orange-600 transition-colors flex-shrink-0"></i>
                                </div>
            ...
            <!-- Info -->
            <div class="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg md:rounded-xl p-4 md:p-6">
                <div class="flex items-start">
                    <div class="w-8 h-8 md:w-10 md:h-10 bg-amber-100 rounded-lg flex items-center justify-center mr-3 md:mr-4 flex-shrink-0">
                        <i class="bx bx-info-circle text-amber-600 text-lg md:text-xl"></i>
                    </div>
                    <div>
                        <h3 class="text-sm md:text-base font-semibold text-gray-900 mb-2">Panel de Preceptor</h3>
                        <div class="text-xs md:text-sm text-gray-700 space-y-1">
                            <p>Gestiona asistencias, inscripciones y mesas de examen desde este panel.</p>
